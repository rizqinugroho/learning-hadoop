package com.rizqinugroho.examples.streaming
...
object EventTimeExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val socketStreamDs = sparkSession.readStream.format("socket").option("host", "localhost").option("port", 50050).load().as[String]

    // read as stock
    val stockDs = socketStreamDs.map(value => {val columns = value.split(",")
